...
with tempfile.TemporaryDirectory() as d:
    ...
    supervisor=(src/'supervisor.sh').read_text().replace('/usr/libexec/motion-white',str(base)).replace('/tmp/motion-white.supervisor',str(lock)).replace('/tmp/motion-white.run',str(run)).replace('/etc/motion-white.conf',str(conf)).replace('sleep 15','sleep 1')
